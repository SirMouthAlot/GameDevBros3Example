...
public class NumberDisplayDefinition : MonoBehaviour
{
    ...
    public ScriptableNumberFont fontObject;
    ...
    public void ConvertToSprites()
    {
        ...
        for (int i = 0; i < _numDigits; i++)
        {
            //Update the digits to reflect the numbers
            transform.GetChild(i).GetComponent<Image>().sprite = fontObject._numberSprites[int.Parse(splitVal[i].ToString())];
        }
